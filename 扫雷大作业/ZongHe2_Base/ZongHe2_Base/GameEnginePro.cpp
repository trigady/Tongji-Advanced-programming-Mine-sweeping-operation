...
Date: 2019/12/29
Description:
定义了程序扭曲游戏模块，用GameEngine命名空间防止同名污染，是你主要需要修改的文件
*****************************************/
...
namespace GameEnginePro {
	...
	double timeStart, timeNow, timeEnd;				 // 定义开始时的时间
	...
	void renderMap() {
		...
		for (size_t i = 0; i < 2 * mapHeight + 1; i++) {
			...
			for (size_t j = 0; j < mapWidth; j++) {
				// 移动至确切的坐标，绘制方格的边界
				MovePos(2 + (SHORT)j * 3, (SHORT)i + 4);
				if (i % 2 == 0) {
					PutString(j % 2 == 0 ? (i == 2 * mapHeight ? (j == mapWidth - 1 ? "￣ " : "￣\\") : "￣\\") : ((mapHeight % 2 == 0) && (i == 0) && (j == mapWidth - 1) ? " " : "  /"));
				}
				else {
					PutString(j % 2 == 1 ? "￣\\" : "  /");
				}
		...
		for (size_t i = 0; i < mapHeight + 1; i++) {
			// 遍历列 绘制地雷
			for (size_t j = 0; j < mapWidth; j++) {
				MovePos(2 + (SHORT)j * 3, (SHORT)i * 2 + 3 + (j % 2 == 0 ? 0 : 1));    // 移动至确切坐标
				const size_t mapIndex = (i - 1) * mapWidth + j; // 确定地图数组的下标
				char numMap[3] = "  ";							// 确定数字字符串
				numMap[0] = '0' + mapCanvas[mapIndex];			// 当mapVisible[mapIndex]为1到8时，将其转换成字符串
		...
		// 绘制最下面一层的底部。
		for (size_t j = 1; j < mapWidth; j = j + 2) {
			// 移动至确切的坐标，绘制方格的边界
			MovePos(2 + (SHORT)j * 3, (SHORT)2 * mapHeight + 5);
			PutString("￣");
		}
		MovePos(0, 2 * mapHeight + 5);
	...
	*****************************************/
	void renderChoiceBackground(COORD choice) {
		...
		MovePos(2 + (SHORT)x * 3, 4 + (((x % 2 == 0) && ((SHORT)y % 2 == 1) ? (SHORT)y * 2 + 1 : (SHORT)y * 2) - 2) / 2 * 2 + 2 + (x % 2 == 0 ? 0 : 1));
	...
	*****************************************/
	void checkChoice() {
		...
		// 将鼠标控制台的坐标放缩至地图坐标
		const size_t mouseX = (size_t)((size_t)pos.X - 1) / 3;
		const size_t mouseY = ((size_t)((size_t)pos.Y - (mouseX % 2 == 0 ? 0 : 1)) / 2) - 2;
	...
	*****************************************/
	void initMap() {
		...
		if ((safeZone % mapWidth) % 2 == 0) {
			safePlace[6] = safePlace[8] = -1;
		}
		else{
			safePlace[0] = safePlace[2] = -1;
		}
